...
| **Gen2 LanPaint Soft Denoise Patch** | Clones a `MODEL` and attaches a keyed `SAMPLER_SAMPLE` wrapper for the four current LanPaint sampler nodes. The binary `noise_mask` remains LanPaint's only algorithm mask. Existing original/latest/EMA input-reference behavior remains available, while an appended opt-in output mode can spatially mix immutable source texture with the previously accepted latest reference and independently decay output preservation over normalized step or sigma progress. New controls default to `legacy`, preserving saved workflows. Supports 4D image latents and explicitly rejects unvalidated 5D video latents. See `docs/LANPAINT_SOFT_DENOISE.md` and `workflows/lanpaint_soft_denoise_template.json`. |
